-- 🏗️ CRÉATION TABLES FOURNISSEUR POUR FINITIO - VERSION CORRIGÉE
-- Ce script crée les tables spécialisées pour les fournisseurs
-- Compatible avec le système users.role = 'fournisseur' existant
...
CREATE POLICY "Fournisseurs gèrent leurs produits" ON produits
  FOR ALL USING (
    fournisseur_id IN (
      SELECT id FROM fournisseurs WHERE user_id IN (
        SELECT id FROM users WHERE id = (
          SELECT auth.uid()
        )
      )
