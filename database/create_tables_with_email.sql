-- 🏗️ CRÉATION TABLES FOURNISSEUR AVEC EMAIL COMME RÉFÉRENCE
-- Script adapté pour utiliser email au lieu de user_id
-- Compatible avec votre structure users existante
...
CREATE TABLE IF NOT EXISTS fournisseurs (
  ...
  user_email text references users(email) on delete cascade unique,
  nom_entreprise text,
  ICE numeric,
  adresse text,
  ville text,
...
CREATE TABLE IF NOT EXISTS commandes (
  id uuid primary key default gen_random_uuid(),
  fournisseur_id uuid references fournisseurs(id) on delete cascade,
  client_email text references users(email) on delete cascade,
  projet_id uuid,
...
CREATE INDEX IF NOT EXISTS idx_fournisseurs_email ON fournisseurs(user_email);
...
CREATE INDEX IF NOT EXISTS idx_commandes_client ON commandes(client_email);
...
SELECT 'Table users mise à jour:' as info, column_name, data_type
...
SELECT 'Structure fournisseurs:' as info, column_name, data_type
FROM information_schema.columns 
