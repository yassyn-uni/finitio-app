-- 🎯 SCRIPT DE GÉNÉRATION DE DONNÉES DE TEST ANALYTICS - VERSION FINALE FONCTIONNELLE
...
-- ✅ 4. GÉNÉRER DES INSCRIPTIONS DE TEST (SANS user_id pour éviter les contraintes)
...
-- ✅ 5. GÉNÉRER DES CONNEXIONS DE TEST (SANS user_id pour éviter les contraintes)
INSERT INTO analytics_logins (user_id, email, login_source, success, error_message, created_at)
VALUES 
  (NULL, 'uniform.outlet@aluxury.ma', 'login_form', true, NULL, NOW() - INTERVAL '30 minutes'),
  (NULL, 'test1@finitio.com', 'login_form', true, NULL, NOW() - INTERVAL '2 hours'),
  (NULL, 'test2@finitio.com', 'login_form', false, 'Invalid credentials', NOW() - INTERVAL '3 hours'),
  (NULL, 'test3@finitio.com', 'login_form', true, NULL, NOW() - INTERVAL '1 day');
...
-- - 3 inscriptions avec noms, méthodes et métadonnées UTM (user_id = NULL)
-- - 4 connexions (3 succès, 1 échec avec message d'erreur) (user_id = NULL)
-- - 3 conversions trackées (signup, login)
-- - 7 vues SQL optimisées pour le dashboard analytics

-- 📝 NOTE: Les user_id sont NULL pour éviter les contraintes de clés étrangères
